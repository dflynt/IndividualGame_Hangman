import random
import os
words = ["phone", "cruise ship", "soccer", "movie",
         "vacation home", "road trip", "tortoise"]
...
while not wordSolved:
    ...
    if choice in word:
        # add index of letter in word to the list if
        # guessed letter is in the word
        indexesContainingGuessedLetter =
        [i for i, x in enumerate(word) if x == choice]

        for x in range(0, len(indexesContainingGuessedLetter)):
            # add correctly guessed letters to their
            # corresponding index in the mystery word
